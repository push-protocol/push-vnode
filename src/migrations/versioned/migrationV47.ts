...
export default async (upgrade) => {
  ...
  const upgradeScript = async (): Promise<boolean> => {
    ...
    `
      ...
      for (const row of w2wRows) {
        const combinedDidArray = row.combined_did.split('_')
        const adminArray = row.admins ? row.admins.split('_') : []
        const intentArray = row.intent ? row.intent.split('+') : []
  ...
  const downgradeScript = async () => {
    crashWithError("Can't downgrade... Version 47 is breaking changes!")
  }
