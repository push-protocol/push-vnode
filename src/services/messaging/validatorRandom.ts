...
import { RandomUtil } from '../../../tests/utilz/randomUtil'
...
export class ValidatorRandom {
  ...
  public createAttestToken(): AttestToken {
    ...
    const t = new AttestToken(NetworkRandom.write(nr), attestVector)
    return t
  }
  ...
  public checkAttestToken(
    ...
    const nr = NetworkRandom.read(attestToken)
    const attestVector = ValidatorRandom.createValidationVector(
      this.log,
      this.contractState.getValidatorNodesMap(),
      this.contractState.contractCli.valPerBlock - 1,
  ...
  public static createValidationVector(
    ...
    const successfulPingCount = this.calculateSuccessfulPingCount(networkRandom)
    log.debug('successfulPingCount: %o', successfulPingCount)
    ...
    for (const nodeRandom of networkRandom.nodes) {
      ...
      const pings = successfulPingCount.get(nodeRandom.nodeId)
      const nodeHasSuccessfulPings =
        nodeRandomPingRequired == 0 || (pings != null && pings >= nodeRandomPingRequired)
      if (
        goodTimestamp &&
        validNodeId &&
        validSignature &&
        validRandomHex &&
        nodeHasSuccessfulPings
      ) {
        ...
        log.error(
          'invalid nodeRandom: %o, ' +
            ...
            'validRandomHex: %o, nodeHasSuccessfulPings: %s',
          nodeRandom,
          goodTimestamp,
          validNodeId,
          validSignature,
  ...
  static calculateValidationVector(
    ...
    if (randomNodesRequired < 1 || randomNodesRequired > 64) {
      throw new Error(
        'current function works only for 64 nodes, rndNodesRequired=' + randomNodesRequired
      )
    }
    ...
    // let's assume that we will always have less than 256 nodes (8 bits per node)
    // so that we have enough randomness to read 64x
...
export class NetworkRandom {
  ...
  public static read(nr: string): NetworkRandom {
    return <NetworkRandom>JSON.parse(BitUtil.base64ToStr(nr))
  }
